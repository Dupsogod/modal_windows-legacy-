function searchOfKeywords(filteredTasks){
    ...
    if(filteredTasks.length){
        var urlRequestNewsList = `${REQUEST_URL}_api/web/lists(guid'${LIST_NEWS_GUID}')/items?$orderby=Modified desc`;
        listTasks = filteredTasks;
        getListNews(urlRequestNewsList)
    }else{
        var myTasks = document.querySelector('.matchingTask').checked;
        var filter = myTasks ? "" : `&$filter=initiator eq '${currentUser.UserName}'`;
        var urlRequestTaslList = `${REQUEST_URL}_api/web/lists(guid'${LIST_TASKS_GUID}')/items?$orderby=Modified desc ${filter}`;
