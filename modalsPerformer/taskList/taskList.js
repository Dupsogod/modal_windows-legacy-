function taskList(filteredTasKlist){
    ...
    if(filteredTasKlist.length){
        showTaskList(filteredTasKlist)
            ...
            var urlGetTaskList = `${REQUEST_URL}_api/web/lists(guid'${LIST_TASKS_GUID}')/items?$orderby=Modified desc&$filter=performer eq '${currentUser.UserName}'`;
            var tasks = [];
        ...
        if(!tasksList.length){
            ...
            tasksList.forEach((task)=>{
                ...
                taskItems += `<div class='task' id="${task.Id}">
                                ...
                                <div class='performer'>${task.performer}</div>
            ...
            listTasksItems.addEventListener('click', e=>{
                    ...
                    //setModal()
                    formTask(e.target.dataset.item)
